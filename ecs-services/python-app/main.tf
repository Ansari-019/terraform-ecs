...
# No of ALB Requests
#####################
resource "aws_appautoscaling_policy" "aws_ecs_policy_alb" {
  name               = "${local.container_name}-autoscaling-${local.environment}"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.aws_ecs_target.resource_id
  scalable_dimension = aws_appautoscaling_target.aws_ecs_target.scalable_dimension
  service_namespace  = aws_appautoscaling_target.aws_ecs_target.service_namespace
  ...
  target_tracking_scaling_policy_configuration {
    predefined_metric_specification {
      ...
      resource_label          = "${data.terraform_remote_state.alb.outputs.alb.arn_suffix}/${data.terraform_remote_state.alb.outputs.alb.target_groups["python-app"].arn_suffix}"
